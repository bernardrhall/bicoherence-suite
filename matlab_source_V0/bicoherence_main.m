...
%GW_channel = 'H1_CAL_DELTAL_EXTERNAL_DQ.csv';
%GW_channel = '1115716480.2725.csv';
%GW_channel = '1114619416.csv';
%GW_channel = '1111225068.902_8.csv';
%GW_channel = 'test_signal.csv';
GW_channel = 'L1-GDS-CALIB_STRAIN_11346440238.csv';
%GW_channel = 'H1-CAL-DELTAL_EXTERNAL_DQ_112625944630.csv';

%GW_channel_1 = 'H1-CAL-DELTAL_EXTERNAL_DQ_112625944630.csv'; % if using time-slide coherence
%GW_channel = 'SRCL.csv';
...
if fnct ~= 7
    ...
    if (fnct == 0 || fnct == 1) && filt == 0
        ...
        fsCheck1 = round(length(chA(:,1))/(GWc(length(chA(:,1))-1,1) - chA(1,1)));
        fsCheck2 = round(length(chB(:,1))/(GWc(length(chB(:,1))-1,1) - chB(1,1)));
...
elseif fnct==1  % cross bicoherence
    ...
    if plot_img == 1
        colormap jet
        imagesc(freqBsp_F,freqBsp_F,bicx)
        GW_channel_n = strrep(GW_channel,'_','\_');
        GW_channel_n = regexprep(GW_channel_n,'\d','');
        GW_channel_n = strsplit(GW_channel_n,'.');
...
%     end
    ...
    clearvars -except bisp freqBsp_F plot_img GW_channel fs GPS_central time_window pass_band_fl pass_band_fh randVec seg seg_length GW_1c tile Snr Sr
    ...
    final_results = bcohCrit( 0.996, bisp, freqBsp_F, str2double(T) );
    %T = bspcThresh(seg,seg_length,GW_1c(lower:upper,1),randVec);
    
    %disp(strcat('T value:...',num2str(T)))
    
    clearvars -except bisp freqBsp_F randVec T Snr Sr final_results GPS_central
...
%     [d,e,f] = bicoherd_single_follow(tMatrix,results,0,1,tiled,nft,hann(seg_length,'periodic'),seg_length,ol); %****
    ...
    if goflag
        ...
        final_results = bcohCrit( 0.7, d, freqBsp_F, str2double(T) );
        %results = 0;
    end
    clearvars -except bisp freqBsp_F fs randVec resultsC final_results d Snr Sr
